...
<b>/godot/native/lib</b>          //compiled gdnative libraries and dependencies (.dll)
...
Call method 1
    ```
        ...
        **Possible errors:**
        ...
        - {:.possible_errors} ```ERROR: Can't open dynamic library: C:/Users/user/Documents/sailing_west/lib/godot-cpp/test/lib/libgdexample.dll, error: Error 126``` - Godot can’t find your compiled dll. Check the path in **gdexample.gnlib** in **Windows.64=** parameter of section **[entry]** and if it contains the correct path to your library
...
From now on I will skip most of navigation (especially back to the project root)
-  Create release directory in build
```
mkdir godot-cpp\build\win64\release && cd godot-cpp\build\win64\release
```
- Modify the CMakeLists.txt in **lib/godot-cpp/**. Disable debug information in the builds so the Godot library build would be not 130Mb, but 5Mb. On line 110 replace:
```cmake
...
There is also a possibility to build a godot-cpp library that can be referenced from the outside library as a .dll, dynamic link library. Why would you do it? If you want to make several modules, statically linking with godot-cpp would mean it’s copied into every library you build. On the downside, you’ll have to distribute not a single gdnative .dll, but several .dll files alongside with it. But you still are distributing your .dll separately from your executable, so why not make even more of a mess?
...
- Test GDNative as a dynamic library with a provided test project
    - Copy all .dll dependencies to the **godot-cpp/test/** folder:
...
## Step 6: Create your own GDNative project
...
- ```cd native/``` then ```mkdir world\src && cd world```
- Copy **init.cpp** into **gdnative/world/src**
...
- {:.continue_list} Test with the same Godot test project
    ...
    - replace **Windows.64="res://libgdexample.dll"** to **Windows.64="res://native/lib/win64/libworld.dll"** in gdexample.gnlib in the section **[entry]**.
    - run the same 
    ```
Godot_v3.3.2-stable_win64.exe -s script.gd
    ```
...
## Step 7: Export godot project
Godot built-in exporter will copy it’s GDNative libraries while exporting, but won’t copy our dependencies. For me the simplest way to fix that is to create a simple python build script. You can use any CI tool for that matter. Now you can try exporting your project from Godot editor to th **export/** directory. If you have statically linked everything into your .dll, then everything runs. If you are trying to link libraries dynamically, then you have to manually copy all the dlls.
- Let’s create build.py in the root folder which will export our game with all it’s dll dependencies. That way we can automate builds for other platforms later. *This code uses **robocopy** Windows utility.*

```python
import configparser
 
EXPORT_PATH = "../export/win64/Native_project.exe"

def update_godot_export_templates():
    export_config = configparser.ConfigParser()
    export_config.read("godot/export_presets.cfg")
    export_config["preset.0"]["export_path"] = "\"%s\"" % EXPORT_PATH
...
if __name__ == "__main__":
    import os
    update_godot_export_templates() # update export templates to use our export path
    os.system("cd godot && Godot_v3.3.2-stable_win64.exe --no-window --export-debug \"Windows Desktop\" %s && cd .." % EXPORT_PATH)
    os.system("robocopy godot/native/lib/win64 export/win64 *.dll")
```

- {:.continue_list} run the script with ```python build.py```
